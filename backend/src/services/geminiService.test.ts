...
describe("GeminiService", () => {
	...
	describe("transcribeAudio", () => {
		...
		it("should handle transcription errors", async () => {
			...
			await expect(
				geminiService.transcribeAudio(audioBuffer, "audio/webm"),
		...
		it("should handle empty response from Gemini", async () => {
			...
			await expect(
				...
				"Failed to transcribe audio: No transcription received from Gemini",
			);
		});
	...
	describe("extractFacts", () => {
		...
		it("should handle extraction errors", async () => {
			...
			await expect(geminiService.extractFacts(transcript)).rejects.toThrow(
				"Failed to extract facts: Gemini extraction failed",
			);
		});
		...
		it("should handle empty response from Gemini", async () => {
			...
			await expect(geminiService.extractFacts(transcript)).rejects.toThrow(
				"Failed to extract facts: No extraction response received from Gemini",
			);
		});
		...
		it("should validate response format", async () => {
			...
			await expect(geminiService.extractFacts(transcript)).rejects.toThrow(
				"Failed to extract facts: Invalid response format from Gemini",
			);
		});
		...
		it("should include career coaching prompt", async () => {
			...
			expect(callArgs.contents).toContain(
				"job seeker trying to build their resume",
			);
	...
	describe("error handling", () => {
		...
		it("should preserve original error messages", async () => {
			...
			await expect(geminiService.extractFacts(transcript)).rejects.toThrow(
				"Failed to extract facts: Specific Gemini error message",
			);
		});
	});
});
