...
import { CreditsService, TopicService, InterviewService } from "@/services";
import { CreditsRepository, TopicRepository, ExperienceRepository, InterviewRepository } from "@/repositories";
...
export class ProcessInterviewWorkflow {
    ...
    private async runWorkflow(userId: string, interviewId: number): Promise<void> {
        ...
        const extractionTokenCount = extractFactsResult.usage.inputTokens + extractFactsResult.usage.outputTokens;

        // Generate Topic Candidates
        const topicCandidatesResult = await this.topicService.generateTopicCandidates(
        ...
        const generationTokenCount = topicCandidatesResult.usage.inputTokens + topicCandidatesResult.usage.outputTokens;
        ...
        const rerankingTokenCount = rerankedTopicsResult.usage.inputTokens + rerankedTopicsResult.usage.outputTokens;
    ...
    private async persistTransaction(
        ...
        this.experienceRepo.saveOrUpdateRecord(
            userId,
            facts,
        );
